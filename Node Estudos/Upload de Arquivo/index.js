...
const servidor = http.createServer((req, res) => {
    if (req.url == '/envioDeArquivo') { //Se a URL tiver /envioDeArquivo ele faz a rotina do tratamento do arquivo
        const form = new formidable.IncomingForm() //retorna o formulário repassado pra ele
        form.parse(req, (erro, campos, arquivos) => {
            const urlantiga = arquivos.filetoupload.path
            const urlnova = 'C:/Users/rafin/OneDrive/Documentos/GitHub/Front-End Estudos/Node Estudos/Upload de Arquivo/Destino Arquivo/' + arquivos.filetoupload.name
        ...
        res.write('<form action="envioDeArquivo" method="post" enctype="post" enctype="multipart/form-data">')
