...
//Aqui, você está importando o módulo Mongoose, que é uma biblioteca para facilitar a interação com o MongoDB usando esquemas e modelos.
const express = require('express');
//Essa linha importa o módulo Express, um framework web para Node.js, que facilita a criação de aplicações web e APIs.
const app = express();
//Aqui, você cria uma instância do aplicativo Express. Essa instância (app) será usada para definir rotas e middleware.
const port = process.env.PORT || 3000;
//Essa linha define a porta em que o servidor irá escutar. Ele primeiro tenta pegar a variável de ambiente PORT (definida no .env) e, se não existir, usa a porta 3000 como padrão
...
async function connectMongoDB() { //Essa função assíncrona é responsável por conectar ao MongoDB usando Mongoose.
  ...
  try {
    ...
    //Essa linha tenta conectar ao MongoDB usando a string de conexão obtida anteriormente. O await é usado porque mongoose.connect retorna uma Promise (uma operação assíncrona) e queremos esperar até que a conexão seja estabelecida.
    console.log('Conectado ao MongoDB com sucesso!');
    
    // Testar conexão enviando um ping
    await mongoose.connection.db.command({ ping: 1 });
    //Esta linha envia um comando de ping para o banco de dados MongoDB, confirmando que a conexão está ativa. É uma maneira de verificar a saúde da conexão.
